...
class _DataPageState extends State<DataPage> {
  ...
  Future<void> _confirmDelete(int id, String code) async {
    ...
    if (confirm == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Data berhasil dihapus'), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating),
      );
    }
  }
  ...
  List<DataColumn> _buildColumns() {
    const textStyle = TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.1, fontSize: 12);
    return [
      ...
      DataColumn(
        ...
        numeric: true,
        onSort: (idx, asc) => _sort((d) => d['reminder_limit'] ?? 0.0, idx, asc)
      ),
      const DataColumn(label: Text('Action', style: textStyle)),
    ];
  }

  DataRow _buildRow(Map<String, dynamic> item, int index, BuildContext context, ColorScheme cs) {
    final int displayIndex = ((_currentPage - 1) * _itemsPerPage) + index + 1;

    return DataRow(cells: [
      DataCell(Text("$displayIndex")), // Kolom Nomor Urut
      DataCell(Text(item['code'].toString(), style: const TextStyle(fontWeight: FontWeight.w600))),
      ...
      DataCell(
        item['is_reminder'] == 1
            ? Container(
                ...
                decoration: BoxDecoration(color: cs.secondaryContainer.withOpacity(0.5), borderRadius: BorderRadius.circular(20)),
                child: Text(item['reminder_limit'].toString(), style: TextStyle(color: cs.onSecondaryContainer, fontSize: 11, fontWeight: FontWeight.bold)),
              )
            : const Text("-"),
      ),
      DataCell(
